								...
								bool process_hypertable_invalidations, bool extend_last_bucket)
{
	 ...
	 *
	 * If a live entry with an overlapping window already exists, we emit a
	 * notice and return without doing any work.
	 */
		...
		ereport(ERROR,
				(errcode(ERRCODE_LOCK_NOT_AVAILABLE),
				 errmsg("could not refresh continuous aggregate \"%s\"",
						NameStr(cagg->data.user_view_name)),
				 errdetail("A concurrent refresh with an overlapping window is already in "
						   "progress.")));
